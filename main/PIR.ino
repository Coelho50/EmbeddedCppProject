...
void standby(){                           //only yellow LED on
  ...
  Serial.println("or enter a character in the serial monitor for log options:");
  Serial.println("1 - Print log list");
  Serial.println("2 - Clear log list");
  Serial.println("3 - Print and clear log list");

  delay(1000);
  while(!reader.PICC_IsNewCardPresent()){   //waits for card/tag aproximation
    if (Serial.available()){
      int cmd = Serial.read();
      log_list.printLog(cmd - 48);
